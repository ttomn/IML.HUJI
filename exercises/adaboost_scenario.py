...
def decision_surface1(predict, xrange, yrange, t, density=120, dotted=False, colorscale=custom, \
                      showscale=True):
...
def fit_and_evaluate_adaboost(noise, n_learners=250, train_size=5000, test_size=500):
    ...
    fig = go.Figure(layout=go.Layout(
        title=rf"$\textbf{{loss of Adaboost running with decision stump as a function of amount of base estimators}}$",
        margin=dict(t=100)))
    ...
    for i, t in enumerate(T):
        fig.add_traces([decision_surface1(learner.partial_predict, lims[0], lims[1], t=t, showscale=False),
    ...
    go.Figure(
        data=[decision_surface1(learner.partial_predict, lims[0], lims[1], t=best_t + 1, showscale=False),
              go.Scatter(x=test_X[:, 0], y=test_X[:, 1], mode="markers", showlegend=False,
                         marker=dict(color=test_y, symbol=symbols[1],
                                     colorscale=[
                                         custom[0],
    ...
    go.Figure(
        data=[decision_surface1(learner.partial_predict, lims[0], lims[1], t=n_learners, showscale=False),
              go.Scatter(x=train_X[:, 0], y=train_X[:, 1], mode="markers", showlegend=False,
                         marker=dict(size=learner.D_/np.max(learner.D_) * 5, color=train_y, symbol=symbols[0],
                                     colorscale=[
                                         custom[0],
                                         custom[-1]],
                                     line=dict(color="black", width=1)))],
...
if __name__ == '__main__':
    np.random.seed(0)
    fit_and_evaluate_adaboost(noise=0, n_learners=250, train_size=5000, test_size=500)
